...
it('invalidates results after source edits but ignores API runtime data', async () => {
  ...
  try {
    for (const path of ['src/data', 'server/data']) await mkdir(join(root, path), { recursive: true });
    for (const path of ['package.json', 'package-lock.json', 'vitest.config.ts', 'vite.config.ts', 'src/data/requirements.ts']) await writeFile(join(root, path), 'initial');
