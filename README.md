...
各6章、全18章です。余裕があれば「そのテスト、不具合を見つけられる？」も開いてみてください。一時的に実装を壊し、テストがちゃんと気付けるかを試す任意の挑戦です。終わったら元に戻すのを忘れずに。

## ブランチの使い分け

- **`master`：** 学習開始用。最新の学習UIとチャプター0の実演は使えますが、タスクの機能と学習用テスト本文は未実装です。
- **`my-study`：** 自分が解き進めたコードとテストを残す場所です。

続きに取り組むときは、変更を保存・コミットしてから `git switch my-study`。最初から別に始めたいときは、`master` から自分用の新しいブランチを作ってください。

ブランチを変えても、ブラウザに保存したチェック・選択章は共通です。実行結果もPC内の `.learning/` に保存されるため、切り替え後はテストを再実行してください。

## ちょっと補足

DBは使っていません。ブラウザは同じPCのExpress（ポート3001）と通信して、テスト項目や結果ファイルを読みます。APIの課題は最初、501「未実装」を返します。ここから自分で作っていきます。
